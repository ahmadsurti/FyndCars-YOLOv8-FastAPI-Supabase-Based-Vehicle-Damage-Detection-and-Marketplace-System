    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <SheetHeader className="sr-only">
          ...
          <SheetDescription>Displays navigation links and options.</SheetDescription>
        </SheetHeader>
        <SheetContent
  ...
  return (
    <div
      ...
      className={cn(
        ...
        "md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ms-0 md:peer-data-[variant=inset]:mt-2 md:peer-data-[variant=inset]:rounded-xl md:peer-data-[variant=inset]:shadow-sm md:peer-data-[variant=inset]:peer-data-[state=collapsed]:ms-2 md:peer-data-[variant=inset]:overflow-hidden",
        className,
      )}
      {...props}
    />
...
function SidebarMenuSkeleton({
  ...
  const width = React.useMemo(() => `${Math.floor(Math.random() * 35) + 55}%`, [])
  return (
    <div
      data-slot="sidebar-menu-skeleton"
      data-sidebar="menu-skeleton"
      className={cn("flex h-8 items-center gap-2 rounded-md px-2 bg-sidebar-accent/30 animate-pulse", className)}
      {...props}
    >
      {showIcon && <Skeleton className="size-3.5 rounded bg-sidebar-foreground/15 shrink-0" data-sidebar="menu-skeleton-icon" />}
      <Skeleton
        className="h-3 rounded bg-sidebar-foreground/15 max-w-(--skeleton-width) flex-1"
...
function SidebarMenuSubButton({
  ...
  return (
    <Comp
      ...
      data-active={isActive}
      className={cn(
        "flex h-7 min-w-0 -translate-x-px items-center gap-2 overflow-hidden rounded-md px-2 text-sidebar-foreground ring-sidebar-ring outline-hidden hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:text-inherit",
