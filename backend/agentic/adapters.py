...
_SEV_RANK = {"minor": 1, "moderate": 2, "severe": 3}


def pick_primary_detection(detections: list[dict]) -> Optional[dict]:
    """Pick most critical detection (highest severity, then highest confidence) in O(N)."""
    if not detections:
        return None
    return max(
        detections,
        key=lambda d: (_SEV_RANK.get(str(d.get("severity", "")).lower(), 0), d.get("confidence", 0.0)),
    )


def detection_to_damage_signal(detection: Dict[str, Any]) -> Dict[str, Any]:
    """Convert CV detection dict to policy-engine signal dict."""
    return {
